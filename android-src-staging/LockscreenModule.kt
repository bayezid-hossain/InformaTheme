...
class LockscreenModule(private val reactContext: ReactApplicationContext) :
    ...
    @ReactMethod
    fun getPendingDeletions(promise: Promise) {
        try {
            val prefs = reactContext.getSharedPreferences(LockscreenService.PREFS_NAME, Context.MODE_PRIVATE)
            val deletions = prefs.getString("pending_deletions", "[]") ?: "[]"
            promise.resolve(deletions)
        } catch (e: Exception) {
            promise.reject("DELETIONS_ERROR", e.message)
        }
    }

    @ReactMethod
    fun clearPendingDeletions(promise: Promise) {
        try {
            val prefs = reactContext.getSharedPreferences(LockscreenService.PREFS_NAME, Context.MODE_PRIVATE)
            prefs.edit().putString("pending_deletions", "[]").apply()
            promise.resolve(true)
        } catch (e: Exception) {
