...
import { useTheme, themes, themeWallpapers } from '../hooks/useTheme';
...
export function LockscreenPreview({ wallpaperUri, filterOverlay, isFullScreen = false }: LockscreenPreviewProps = {}) {
  const { colors, variant, selectedWallpaper, customWallpaper } = useTheme();
  ...
  const activeFilter = filterOverlay || (customWallpaper ? getFilterOverlay(customWallpaper.filter) : null);
    ...
    <>
      ...
      <View 
        style={[
          ...
          { 
            backgroundColor: (variant === 'warmLight' || variant === 'softSage') 
      ...
      <View style={{ flex: 1, paddingHorizontal: 16, paddingTop: isFullScreen ? 60 : 20, paddingBottom: isFullScreen ? 110 : 24 }}>
        ...
        <View style={{ flexDirection: 'row', gap: 8, marginBottom: 8 }}>
          <View 
            style={{ 
              ...
              paddingVertical: 6, 
              borderRadius: 16, 
              backgroundColor: (variant === 'warmLight' || variant === 'softSage') ? 'rgba(0,0,0,0.05)' : 'rgba(0,0,0,0.2)', 
              borderWidth: 1, 
              borderColor: (variant === 'warmLight' || variant === 'softSage') ? 'rgba(0,0,0,0.1)' : 'rgba(255,255,255,0.1)' 
            }}
          >
